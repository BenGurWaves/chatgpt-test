export async function onRequestPost(context) {
  try {
    ...
    const body = new URLSearchParams({ mode: 'payment', 'line_items[0][price_data][currency]': 'usd', 'line_items[0][price_data][unit_amount]': String(amount), 'line_items[0][price_data][product_data][name]': name, 'line_items[0][quantity]': '1', success_url: `${origin}/success.html?session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${origin}/#pricing`, customer_creation: 'always', 'metadata[plan]': plan });
    ...
    if (!response.ok) return Response.json({ error: data.error?.message || 'Stripe error' }, { status: 500 });
